...
_BIAS_BUCKETS = ("LEFT", "CENTER", "RIGHT")
    ...
    for key in keys:
        ...
        try:
            ...
            if candidate_payload.get("status") != "ok":
                _record_provider_attempt("currents", key, success=False)
                logger.warning(
                    "Currents returned non-ok status with key %s",
                    _mask_key(key),
                )
                continue
            _record_provider_attempt("currents", key, success=True)
            payload = candidate_payload
            break
        except requests.HTTPError as exc:
            status_code = exc.response.status_code if exc.response is not None else None
    ...
    *,
    ...
    for key in keys:
        ...
        try:
            ...
            candidate_payload = resp.json()
            if candidate_payload.get("status") != "ok":
                _record_provider_attempt("newsapi", key, success=False)
                logger.warning(
                    "NewsAPI returned non-ok status with key %s",
                    _mask_key(key),
                )
                continue
            _record_provider_attempt("newsapi", key, success=True)
            payload = candidate_payload
            break
        except requests.HTTPError as exc:
            status_code = exc.response.status_code if exc.response is not None else None
...
def _redistribute_unrated_biases(articles: list[NormalArticle]) -> list[NormalArticle]:
    """
    Reassign UNKNOWN-bias articles into LEFT/CENTER/RIGHT buckets.

    Rules:
      1) Preserve already-classified LEFT/RIGHT/CENTER articles.
      2) Fill any missing bucket first.
      3) Distribute remaining unrated articles to the least represented side.
    """
    if not articles:
        return articles

    counts = {"LEFT": 0, "CENTER": 0, "RIGHT": 0}
    unrated_indexes: list[int] = []

    for index, article in enumerate(articles):
        bias = article.get("bias")
        if bias in counts:
            article["bias_inferred"] = False
            counts[bias] += 1
            continue
        unrated_indexes.append(index)

    if not unrated_indexes:
        return articles

    # Prioritize filling missing sides first.
    missing_buckets = [bucket for bucket in _BIAS_BUCKETS if counts[bucket] == 0]
    assign_cursor = 0

    for bucket in missing_buckets:
        if assign_cursor >= len(unrated_indexes):
            break
        article_index = unrated_indexes[assign_cursor]
        articles[article_index]["bias"] = bucket
        articles[article_index]["bias_inferred"] = True
        counts[bucket] += 1
        assign_cursor += 1

    # Then balance remainder. Tie-breaker prefers LEFT/RIGHT before CENTER.
    balance_priority = {"LEFT": 0, "RIGHT": 1, "CENTER": 2}
    while assign_cursor < len(unrated_indexes):
        article_index = unrated_indexes[assign_cursor]
        target_bucket = min(
            _BIAS_BUCKETS,
            key=lambda bucket: (counts[bucket], balance_priority[bucket]),
        )
        articles[article_index]["bias"] = target_bucket
        articles[article_index]["bias_inferred"] = True
        counts[target_bucket] += 1
        assign_cursor += 1
...
def aggregate_search(
    ...
    *,
    ...
    results = _redistribute_unrated_biases(results)
