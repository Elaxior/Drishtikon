...
class Settings(BaseModel):
    ...
    search_latency_budget_seconds: float = 9.5
    search_max_query_variants: int = 2
    social_extraction_timeout_seconds: float = 6.0
    ...
    # Verification pipeline (default-on)
    enable_verifier: bool = True
    verification_label: str = "Verified Consensus"
    verification_claim_limit: int = 8
    verification_cache_ttl_seconds: int = 900
    verification_evidence_limit: int = 3
    verification_similarity_threshold: float = 0.58
    verifier_fail_open: bool = True
...
settings = Settings(
    ...
    newsdata_daily_limit=_env_int("NEWSDATA_DAILY_LIMIT", 0, 0, 1_000_000),
    gnews_api_key=os.getenv("GNEWS_API_KEY"),
    gnews_daily_limit=_env_int("GNEWS_DAILY_LIMIT", 100, 0, 1_000_000),
    currents_api_key=os.getenv("CURRENTS_API_KEY"),
    currents_daily_limit=_env_int("CURRENTS_DAILY_LIMIT", 600, 0, 1_000_000),
    newsapi_api_key=os.getenv("NEWSAPI_API_KEY"),
    newsapi_daily_limit=_env_int("NEWSAPI_DAILY_LIMIT", 100, 0, 1_000_000),
    ...
    provider_timeout_seconds=_env_int("PROVIDER_TIMEOUT_SECONDS", 5, 2, 20),
    search_latency_budget_seconds=_env_float("SEARCH_LATENCY_BUDGET_SECONDS", 9.5, 4.0, 30.0),
    search_max_query_variants=_env_int("SEARCH_MAX_QUERY_VARIANTS", 2, 1, 5),
    social_extraction_timeout_seconds=_env_float("SOCIAL_EXTRACTION_TIMEOUT_SECONDS", 6.0, 0.5, 15.0),
    claim_extraction_timeout_seconds=_env_float("CLAIM_EXTRACTION_TIMEOUT_SECONDS", 3.0, 1.0, 20.0),
    summary_timeout_seconds=_env_float("SUMMARY_TIMEOUT_SECONDS", 2.5, 1.0, 20.0),
    ...
    enable_verifier=_env_bool("ENABLE_VERIFIER", True),
    verification_label=os.getenv("VERIFICATION_LABEL", "Verified Consensus"),
    verification_claim_limit=_env_int("VERIFICATION_CLAIM_LIMIT", 8, 1, 12),
    verification_cache_ttl_seconds=_env_int("VERIFICATION_CACHE_TTL_SECONDS", 900, 0, 86400),
    verification_evidence_limit=_env_int("VERIFICATION_EVIDENCE_LIMIT", 3, 1, 6),
    verification_similarity_threshold=_env_float("VERIFICATION_SIMILARITY_THRESHOLD", 0.58, 0.4, 0.95),
    verifier_fail_open=_env_bool("VERIFIER_FAIL_OPEN", True),
