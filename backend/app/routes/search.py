...
from app.services.consensus import calculate_consensus, generate_consensus_report, group_claims
from app.services.news_aggregator import aggregate_search, aggregate_trending
...
def _select_balanced_articles(raw_articles: list[dict[str, Any]], limit: int) -> list[dict[str, Any]]:
    ...
    # Add additional known-bias items by quality (not forced equalization).
    known_candidates: list[dict[str, Any]] = []
    for bias in BIAS_BUCKETS:
        known_candidates.extend(grouped[bias])

    known_candidates.sort(key=_article_quality_key, reverse=True)
...
def _enforce_full_spectrum_labels(articles: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], bool]:
    """
    Guarantee LEFT/CENTER/RIGHT presence when possible.

    Priority:
      1) Reassign articles whose bias was inferred from unrated sources.
      2) If still missing, reassign from the most represented bucket as a fallback.
    """
    if len(articles) < 3:
        return articles, False

    counts = _bias_counts(articles)
    if all(counts[bias] > 0 for bias in BIAS_BUCKETS):
        return articles, False

    adjusted = False

    for missing_bias in [bias for bias in BIAS_BUCKETS if counts[bias] == 0]:
        donor_index: int | None = None

        # First, move an inferred-bias article from a bucket that has spare capacity.
        inferred_candidates: list[tuple[int, str, int]] = []
        for index, article in enumerate(articles):
            current_bias = article.get("bias")
            if current_bias not in BIAS_BUCKETS:
                continue
            if counts[current_bias] <= 1:
                continue
            if article.get("bias_inferred") is True:
                inferred_candidates.append((index, current_bias, counts[current_bias]))

        if inferred_candidates:
            inferred_candidates.sort(key=lambda item: item[2], reverse=True)
            donor_index = inferred_candidates[0][0]

        # Fallback to any side with >1 article if no inferred candidate exists.
        if donor_index is None:
            donor_bias = max(BIAS_BUCKETS, key=lambda bias: counts[bias])
            if counts[donor_bias] > 1:
                donor_index = next(
                    (i for i, a in enumerate(articles) if a.get("bias") == donor_bias),
                    None,
                )
        ...
        donor_bias = articles[donor_index].get("bias")
        if donor_bias not in BIAS_BUCKETS:
            continue

        articles[donor_index]["bias"] = missing_bias
        articles[donor_index]["bias_inferred"] = True
        counts[donor_bias] -= 1
        counts[missing_bias] += 1
        adjusted = True

    return articles, adjusted
...
def search_news(q: str = Query(..., min_length=1)) -> dict[str, Any]:
    ...
    if is_social_media_claim:
        ...
        social_timeout = min(
            settings.social_extraction_timeout_seconds,
            max(1.0, settings.search_latency_budget_seconds * 0.45),
        )
        if social_timeout < settings.social_extraction_timeout_seconds:
            degraded_steps.append("social_extraction_timeout_capped")
        try:
            social_media_data = social_future.result(timeout=social_timeout)
    ...
    if not raw_articles:
        if is_social_media_claim:
            degraded_steps.append("emergency_trending_fallback_attempted")
            emergency_articles = aggregate_trending(
                category="top",
                size_per_provider=4,
            )
            raw_articles = _merge_unique_articles(raw_articles, emergency_articles)

        if not raw_articles:
            degraded_steps.append("emergency_query_fallback_attempted")
            emergency_query_articles = aggregate_search(
                "breaking world news",
                category="top",
                size_per_provider=4,
            )
            raw_articles = _merge_unique_articles(raw_articles, emergency_query_articles)
    ...
    selected_articles, spectrum_adjusted = _enforce_full_spectrum_labels(selected_articles)
    if spectrum_adjusted:
        fallback_used = True
        coverage_warning = (
            "Bias balancing applied to ensure LEFT/CENTER/RIGHT comparison for this query."
        )

    # Build a source → bias lookup for fast claim attribution
    source_bias_map: dict[str, str] = {}
    for item in selected_articles:
        src = item.get("source")
        if isinstance(src, str) and src.strip():
            source_bias_map.setdefault(src.strip(), item.get("bias", "UNKNOWN"))
    ...
    for article in articles:
        ...
        bias = source_bias_map.get(source, article.get("bias", "UNKNOWN"))
        ...
        for claim in claims_value[: settings.consensus_claims_per_article]:
            ...
            all_claims.append({"source": source, "claim": normalized_claim, "bias": bias})

    # Run summary generation in parallel — don't block claim grouping
    ...
    if settings.enable_verifier and claim_groups and _remaining_budget_seconds(started_at) >= 0.9:
        ...
        # Parallelize verification across claim groups
        verify_executor = ThreadPoolExecutor(max_workers=min(4, verification_limit))
        verify_futures: dict[int, Any] = {}
        try:
            for index, group in enumerate(claim_groups):
                if index < verification_limit and _remaining_budget_seconds(started_at) >= 0.35:
                    verify_futures[index] = verify_executor.submit(
                        verify_claim_group,
                        str(group.get("representative_claim") or ""),
                        articles,
                    )

            for index, group in enumerate(claim_groups):
                enriched_group = dict(group)
                if index in verify_futures:
                    remaining = max(0.1, _remaining_budget_seconds(started_at))
                    try:
                        verification_payload = verify_futures[index].result(timeout=remaining)
                    except FuturesTimeoutError:
    ...
    # Collect summary result now (it was running in parallel)
    if summary_future is not None:
        try:
            summary_timeout = min(
                settings.summary_timeout_seconds,
                max(0.5, _remaining_budget_seconds(started_at) - 0.2),
            )
            summary_candidate = summary_future.result(timeout=max(0.1, summary_timeout))
    ...
    consensus_report = generate_consensus_report(
        claim_groups, verification, coverage_data, consensus,
    )
