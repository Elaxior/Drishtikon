		...
		if status_code == 200 and isinstance(payload, dict):
			...
			if status == "completed":
				...
				if isinstance(content, str):
					return _safe_claim_text(content)
				if isinstance(content, list):
					return _safe_claim_text(" ".join(_safe_claim_text(item.get("text")) for item in content if isinstance(item, dict)))
			if status == "failed":
				return ""
		time.sleep(1)
...
def _fetch_supadata_instagram_context(url: str) -> dict[str, str]:
	if not _supadata_enabled() or not _INSTAGRAM_REEL_PATTERN.search(url):
		return {}
	...
	if metadata_status == 200 and isinstance(metadata_payload, dict):
		title = _safe_claim_text(metadata_payload.get("title"))
		description = _safe_claim_text(metadata_payload.get("description"))
	...
	if transcript_status == 200 and isinstance(transcript_payload, dict):
		content = transcript_payload.get("content")
		if isinstance(content, str):
			transcript_text = _safe_claim_text(content)
		elif isinstance(content, list):
			transcript_text = _safe_claim_text(" ".join(_safe_claim_text(item.get("text")) for item in content if isinstance(item, dict)))
	elif transcript_status == 202 and isinstance(transcript_payload, dict):
		job_id = _safe_claim_text(transcript_payload.get("jobId"))
		transcript_text = _supadata_poll_transcript(job_id)

	if transcript_text:
		context["transcript"] = transcript_text[:1600]
		context["source"] = "supadata"
...
def extract_claim_from_social_media(input_text: str, platform: str | None = None) -> dict[str, Any]:
	...
	supadata_metadata = _fetch_supadata_instagram_context(social_url or "") if social_url else {}
