...
def _get_embedding_model() -> Any | None:
    ...
    if _model is None:
        with _model_lock:
            if _model is None:
                _model = SentenceTransformer(settings.pinecone_embedding_model)
...
def _embed_claims(claims: list[str]) -> list[list[float]] | None:
    ...
    try:
        ...
        first_dim = len(vectors[0]) if hasattr(vectors[0], "__len__") else 0
        if first_dim != settings.pinecone_embedding_dimension:
            logger.warning(
                "Embedding dimension mismatch: model produced %s, expected %s",
                first_dim,
