...
def _query_from_social_url(social_url: str | None, platform: str | None) -> str:
	if not social_url:
		return ""

	platform_label = _safe_claim_text(platform or "social media").lower()
	...
	if parsed is not None:
		if "youtube" in (parsed.netloc or "") or "youtu.be" in (parsed.netloc or ""):
			...
			if isinstance(query_params.get("v"), list) and query_params.get("v"):
				video_id = _safe_claim_text(query_params["v"][0])
			if not video_id and parsed.path:
				video_id = _safe_claim_text(parsed.path.strip("/"))

			if video_id:
				return f"youtube {video_id} news update"

	host = _safe_claim_text((parsed.netloc if parsed else "") or "")
	host = host.replace("www.", "")
	if host:
		return _clean_for_query(f"{platform_label} {host} news")

	return _clean_for_query(f"{platform_label} breaking news")
...
def _build_fallback_candidates(
	...
	def add(candidate: str) -> None:
		normalized = _clean_for_query(candidate)
		if not normalized:
			return
	...
	add(metadata.get("description", ""))
	add(metadata.get("transcript", ""))
	add(_query_from_social_url(social_url, platform))
	add(original_input)
	add(f"{_safe_claim_text(platform or 'social')} breaking news")
...
def _fallback_search_query(
	...
	from_url = _query_from_social_url(social_url, platform)
	if from_url:
		return from_url[:220]

	from_input = _clean_for_query(original_input)
	if from_input:
		return from_input[:220]

	platform_label = _safe_claim_text(platform or "social media")
	return _clean_for_query(f"{platform_label} breaking news")[:220] or "latest breaking news"


def _build_derived_search_query(
	extracted_claim: str,
	original_input: str,
	metadata: dict[str, str],
	social_url: str | None = None,
	...
	return _fallback_search_query(original_input, metadata, social_url, platform)[:220]
...
def extract_claim_from_social_media(input_text: str, platform: str | None = None) -> dict[str, Any]:
	...
	fallback_query = _fallback_search_query(original_input, metadata, social_url, platform)
	fallback_search_queries = _build_fallback_candidates(original_input, metadata, social_url, platform)
	if fallback_query:
		fallback_search_queries = [fallback_query, *fallback_search_queries]

	# Keep ordered unique candidates
	unique_candidates: list[str] = []
	seen_candidates: set[str] = set()
	for candidate in fallback_search_queries:
		key = candidate.strip().lower()
		if not key or key in seen_candidates:
			continue
		seen_candidates.add(key)
		unique_candidates.append(candidate[:220])
	fallback_search_queries = unique_candidates[:6]
	...
	derived_search_query = _build_derived_search_query(
		extracted_claim,
		original_input,
		metadata,
		social_url,
	...
	return {
		...
		"fallback_search_queries": fallback_search_queries,
		"is_social_media": True,
		"success": bool(derived_search_query),
	}
