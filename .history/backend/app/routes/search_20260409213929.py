...
def search_news(q: str = Query(..., min_length=1)) -> dict[str, Any]:
    ...
    for candidate_query in _query_variants(effective_query):
        candidate_articles = aggregate_search(
            candidate_query,
            category=None,
            size_per_provider=settings.search_size_per_provider,
        )
        raw_articles = _merge_unique_articles(raw_articles, candidate_articles)
        if _has_enough_articles(raw_articles, settings.analysis_article_limit * 2):
            break
    ...
    if not _has_full_spectrum(selected_articles):
        supplemental_articles = aggregate_search(
            effective_query,
            category=None,
            size_per_provider=max(14, settings.search_size_per_provider),
        )
    ...
    articles = []
    for index, item in enumerate(selected_articles):
        ...
        # Keep analysis source count high without multiplying LLM costs.
        if index < settings.claims_article_limit:
            try:
