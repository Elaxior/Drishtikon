...
def search_news(q: str = Query(..., min_length=1)) -> dict[str, Any]:
    ...
    if is_social_media_claim:
        ...
        try:
            ...
            social_media_data = {
                ...
                "success": False,
                "error": "Social extraction timed out. Falling back to original query.",
            }
        except Exception:
            ...
            social_media_data = {
                ...
                "is_social_media": True,
                "success": False,
                "error": "Social extraction failed. Falling back to original query.",
            }
        finally:
    ...
    if is_social_media_claim:
        variant_limit = max(3, variant_limit)

    candidate_queries = _query_variants(effective_query)[:variant_limit]
    for candidate in social_fallback_queries:
        if len(candidate_queries) >= variant_limit:
            break
        if candidate.lower() not in {q.lower() for q in candidate_queries}:
            candidate_queries.append(candidate)

    if not candidate_queries:
        candidate_queries = [effective_query]
