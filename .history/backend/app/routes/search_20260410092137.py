...
def search_news(q: str = Query(..., min_length=1)) -> dict[str, Any]:
    ...
    if not _has_full_spectrum(selected_articles):
        supplemental_articles = aggregate_search(
            effective_query,
            category=None,
            size_per_provider=max(settings.search_size_per_provider + 2, 12),
        )
    ...
    if claim_target_count > 0:
        with ThreadPoolExecutor(max_workers=settings.claim_extraction_workers) as executor:
