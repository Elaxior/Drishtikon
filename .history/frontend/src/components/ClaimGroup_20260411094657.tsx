    ...
    <div className="rounded-xl border border-parchment-200 bg-parchment/70 p-4 transition-all duration-200 hover:border-parchment-300 dark:border-[#202631] dark:bg-[#151922]/60 dark:hover:border-[#2a313d]">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        {typeof confidence === "number" && (
          <span className="text-xs text-ink-muted dark:text-[#8892a3]">Confidence {Math.max(0, Math.min(100, Math.round(confidence)))}%</span>
        )}
      </div>
          ...
          {showEvidence && (
            <div className="mt-2 space-y-2">
              {evidence.map((item, index) => (
                <div key={`${item.link ?? item.title ?? "evidence"}-${index}`} className="rounded-lg border border-parchment-200 bg-white/80 p-2 dark:border-[#202631] dark:bg-[#111317]/70">
                  <p className="text-[11px] leading-relaxed text-ink-secondary dark:text-[#adb7c7]">{item.snippet}</p>
                  <div className="mt-1 flex items-center gap-2 text-[10px] text-ink-muted dark:text-[#8892a3]">
                    ...
                    {item.link && (
                      <a href={item.link} target="_blank" rel="noreferrer" className="ml-auto text-[10px] font-semibold text-ink-secondary hover:text-ink dark:text-[#adb7c7] dark:hover:text-[#eef1f8]">
                        Open source
                      </a>
                    )}
                  </div>
