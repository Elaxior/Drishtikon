...
const PDF_EXPORT_TIMEOUT_MS = 90000;
...
function resolveHtml2PdfFactory(): Html2PdfFactory | null {
  const imported = html2pdf as unknown;
  if (typeof imported === "function") {
    return imported as Html2PdfFactory;
  }
  ...
  if (
    ...
    return (imported as { default: Html2PdfFactory }).default;
  }

  if (typeof window !== "undefined") {
    const globalFactory = (window as unknown as { html2pdf?: unknown }).html2pdf;
    if (typeof globalFactory === "function") {
      return globalFactory as Html2PdfFactory;
    }
  }
...
function createExportClone(sourceNode: HTMLElement): { exportNode: HTMLElement; cleanup: () => void } {
  const host = document.createElement("div");
  host.setAttribute("data-export-clone", "true");
  host.style.position = "fixed";
  host.style.left = "-100000px";
  host.style.top = "0";
  host.style.width = `${Math.max(sourceNode.scrollWidth, 960)}px`;
  ...
  // Remove media nodes that frequently cause canvas taint/CORS failures.
  clone.querySelectorAll("img,video,iframe,canvas").forEach((mediaNode) => {
    const placeholder = document.createElement("div");
    placeholder.className = "rounded-lg border border-parchment-300 bg-parchment-100 px-3 py-2 text-xs text-ink-muted";
    placeholder.textContent = "Media omitted in PDF export";
...
function Analysis() {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const html2pdfFactory = resolveHtml2PdfFactory();
      if (!html2pdfFactory) {
        throw new Error("html2pdf unavailable");
      }

      const { exportNode: clonedExportNode, cleanup } = createExportClone(exportNode);

      const runExport = (options: Record<string, unknown>) => html2pdfFactory()
        .from(clonedExportNode)
        .set(options)
        .save();

      const baseOptions = {
        margin: [10, 10, 10, 10],
        filename: `drishtikon-report-${querySlug}-${datePart}.pdf`,
        image: { type: "jpeg", quality: 0.96 },
        html2canvas: { scale: 2, useCORS: true, imageTimeout: 20000, logging: false },
      ...
      const fallbackOptions = {
        ...
        html2canvas: { scale: 1.35, useCORS: true, allowTaint: true, imageTimeout: 10000, logging: false },
      };

      const exportTask = runExport(baseOptions).catch(() => runExport(fallbackOptions));

      let timeoutId: number | undefined;
      const timeoutTask = new Promise<never>((_, reject) => {
        timeoutId = window.setTimeout(() => reject(new Error("PDF export timed out")), PDF_EXPORT_TIMEOUT_MS);
      });
      ...
      try {
        console.warn("PDF export failed, falling back to print dialog", error);
        window.print();
        setExportStatus("done");
      } catch {
        setExportStatus("failed");
