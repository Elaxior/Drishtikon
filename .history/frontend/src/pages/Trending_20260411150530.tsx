...
function Trending() {
  ...
  const onViewAnalysis = (item: TrendingItem) => {
    const query = (item.title ?? "").trim() || item.keyword.trim();
  ...
  const handleToggleSave = async (item: TrendingItem) => {
    const saveInput = toSaveArticleInputFromTrending(item, "trending");
                ...
                {sectionItems.length === 0 ? (
                  ...
                  <>
                    <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
                      {featured && (
                        (() => {
                          const saveInput = toSaveArticleInputFromTrending(featured, "trending");
                          const articleId = getSavedArticleId(saveInput);
                          return (
                        <ArticleCard
                          ...
                          onToggleSave={() => void handleToggleSave(featured)}
