...
function Analysis() {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const referencedArticles = data.articles.slice(0, 8);
      ...
      doc.setFontSize(13);
      ensureSpace(8);
      doc.text("Articles Referenced", margin, cursorY);
      cursorY += 7;
      ...
      if (referencedArticles.length === 0) {
        writeWrapped("No article references available.", pageWidth - margin * 2, 6);
      } else {
        referencedArticles.forEach((article, index) => {
          ...
          writeWrapped(`${index + 1}. ${title}`, pageWidth - margin * 2, 5.6);
          writeWrapped(`   Source: ${source} | Bias: ${bias}`, pageWidth - margin * 2, 5.2);
          cursorY += 1;
        });

        if (data.articles.length > referencedArticles.length) {
          writeWrapped(
            `...and ${data.articles.length - referencedArticles.length} more articles.`,
            pageWidth - margin * 2,
            5.2,
          );
        }
