...
function Analysis() {
  ...
  const claimExtractionByBias = useMemo(() => {
    ...
    const seen: Record<"LEFT" | "CENTER" | "RIGHT", Set<string>> = {
      LEFT: new Set<string>(),
    ...
    data.claim_groups.forEach((group) => {
      ...
      const biases = new Set<"LEFT" | "CENTER" | "RIGHT">();
      ...
      if (biases.size === 0 && group.evidence?.length) {
        group.evidence.forEach((item) => {
          if (item.bias === "LEFT" || item.bias === "CENTER" || item.bias === "RIGHT") {
            biases.add(item.bias);
          }
        });
      }

      biases.forEach((bias) => {
        if (!seen[bias].has(claim)) {
          seen[bias].add(claim);
          buckets[bias].push(claim);
        }
      });
    });
