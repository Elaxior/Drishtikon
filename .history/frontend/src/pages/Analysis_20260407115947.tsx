                ...
                <div className="flex-1">
                  ...
                  {data.providers && (
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      ...
                      {data.provider_summary?.partial && (
                        <span className="inline-flex items-center rounded-full border border-amber-500/25 bg-amber-500/10 px-2.5 py-0.5 text-[10px] font-semibold text-amber-700 dark:text-amber-300">
                          {data.provider_summary.text}
                        </span>
                      )}

                      {data.cache?.hit && (
                        <span className="inline-flex items-center rounded-full border border-slate-500/25 bg-slate-500/10 px-2.5 py-0.5 text-[10px] font-semibold text-slate-600 dark:text-slate-300">
                          Cached ({data.cache.age_seconds}s old)
                        </span>
                      )}

                      {data.query_used && data.query_used !== data.query && (
                        <span className="inline-flex items-center rounded-full border border-cyan-500/25 bg-cyan-500/10 px-2.5 py-0.5 text-[10px] font-semibold text-cyan-700 dark:text-cyan-300">
                          Query fallback used
                        </span>
                      )}
                    </div>
