      ...
      <section className="mx-auto max-w-7xl space-y-6">
        <header className="surface-panel rounded-[2rem] px-6 py-8 sm:px-8">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="hidden h-24 w-24 items-center justify-center overflow-hidden rounded-full border border-parchment-300 bg-white/80 text-2xl font-bold text-ink-secondary shadow-sm dark:border-[#2a313d] dark:bg-[#151922]/80 dark:text-[#adb7c7] sm:flex">
              {user?.photoURL ? (
          ...
          {isSavedArticlesLoading ? (
            ...
            <div className="space-y-3">
              {savedArticles.map((article) => {
                const openQuery = article.searchQuery?.trim() || article.keyword?.trim() || article.title?.trim() || "";
                  ...
                  <article key={article.id} className="rounded-xl border border-parchment-200 bg-parchment/65 p-4 dark:border-[#202631] dark:bg-[#111317]/65">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-ink-muted dark:text-[#8892a3]">
                      ...
                      {openQuery && (
                        <button
                          type="button"
                          onClick={() => navigate(`/analysis?q=${encodeURIComponent(openQuery)}`)}
