    ...
    <>
      ...
      <main className="px-4 pb-14 pt-6">
        <section className="mx-auto max-w-7xl space-y-10">
          <header className="surface-panel animate-fade-in-scale overflow-hidden rounded-[2rem] px-5 py-8 sm:px-8 md:px-10 md:py-10">
            <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
              <div>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3 text-xs">
                  <span className={`inline-flex items-center gap-1.5 ${isBackendConnected ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-300"}`}>
                    <span className={`h-1.5 w-1.5 rounded-full ${isBackendConnected ? "animate-pulse bg-emerald-500" : "bg-amber-500"}`} />
                    {connectionMessage}
                  </span>
                  {searchError && <span className="text-red-600 dark:text-red-400">{searchError}</span>}
                  {["NewsData", "GNews", "Currents", "NewsAPI"].map((provider) => (
                    <span key={provider} className="inline-flex items-center gap-1 rounded-full border border-parchment-300 bg-parchment-dark px-2.5 py-1 font-semibold uppercase tracking-wide text-ink-secondary dark:border-[#2a313d] dark:bg-[#151922] dark:text-[#adb7c7]">
              ...
              <div className="surface-panel-soft rounded-3xl p-5 dark:border-[#2a313d]">
                <div className="mb-2 inline-flex items-center gap-1 rounded-full border border-parchment-300 bg-white/80 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-ink-secondary dark:border-[#2a313d] dark:bg-[#111317] dark:text-[#adb7c7]">
          ...
          {trendingLoading ? (
            ...
            <>
              ...
              <div className="flex items-center gap-3 overflow-x-auto rounded-full border border-parchment-300 bg-white/80 px-4 py-2 text-xs text-ink-secondary dark:border-[#2a313d] dark:bg-[#151922]/70 dark:text-[#adb7c7]">
                <span className="shrink-0 font-semibold uppercase tracking-wide text-ink dark:text-[#eef1f8]">In The Loop</span>
                {trendingTopics.slice(0, 8).map((topic, index) => (
                  <button key={`ticker-${topic}-${index}`} onClick={() => handleTopicClick(topic)} className="inline-flex shrink-0 items-center gap-1 hover:text-ink dark:hover:text-[#eef1f8]">
