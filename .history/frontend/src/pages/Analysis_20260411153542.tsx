...
import { searchNews, type ClaimGroup as ApiClaimGroup, type SearchArticle, type SearchResponse, type ConsensusReport } from "../lib/api";
...
type SeedArticleState = {
  seedArticle?: Partial<SearchArticle>;
};
...
function Analysis() {
  ...
  const seededArticle = useMemo(() => {
    const state = location.state as SeedArticleState | null;
    const seed = state?.seedArticle;
    if (!seed || !seed.title) return null;

    return {
      title: seed.title ?? null,
      description: seed.description ?? null,
      source: seed.source ?? null,
      bias: (seed.bias as BiasType) ?? "UNKNOWN",
      claims: Array.isArray(seed.claims) ? seed.claims : [],
      link: seed.link ?? null,
      pubDate: seed.pubDate ?? null,
  ...
  type ClaimPoint = { claim: string; sources: string[]; weight: number };

  const claimExtractionByBias = useMemo(() => {
    const buckets: Record<"LEFT" | "CENTER" | "RIGHT", ClaimPoint[]> = {
      LEFT: [],
      CENTER: [],
      RIGHT: [],
    };
    ...
    const claimData = new Map<string, { counts: Record<"LEFT" | "CENTER" | "RIGHT", number>; sources: Record<"LEFT" | "CENTER" | "RIGHT", Set<string>> }>();
    ...
    data.articles.forEach((article) => {
      ...
      article.claims.forEach((rawClaim) => {
        ...
        const entry = claimData.get(claim) ?? {
          counts: { LEFT: 0, CENTER: 0, RIGHT: 0 },
          sources: { LEFT: new Set(), CENTER: new Set(), RIGHT: new Set() },
        };
        entry.counts[articleBias] += 1;
        entry.sources[articleBias].add(sourceName);
        claimData.set(claim, entry);
      });
    });

    const weightedBuckets: Record<"LEFT" | "CENTER" | "RIGHT", ClaimPoint[]> = {
      LEFT: [],
      CENTER: [],
      RIGHT: [],
    };

    claimData.forEach((entry, claim) => {
      const targetBias = pickDominantBias(entry.counts);
      const weight = entry.counts.LEFT + entry.counts.CENTER + entry.counts.RIGHT;
      const sources = Array.from(entry.sources[targetBias]);
      weightedBuckets[targetBias].push({ claim, sources, weight });
    });

    biasOrder.forEach((bias) => {
      weightedBuckets[bias].sort((a, b) => b.weight - a.weight);
      buckets[bias] = weightedBuckets[bias];
    });
    ...
    data.claim_groups.forEach((group) => {
      ...
      const srcsByBias: Record<"LEFT" | "CENTER" | "RIGHT", Set<string>> = { LEFT: new Set(), CENTER: new Set(), RIGHT: new Set() };

      group.sources.forEach((source) => {
        const bias = sourceBiasMap.get(source);
        if (bias === "LEFT" || bias === "CENTER" || bias === "RIGHT") {
          counts[bias] += 1;
          srcsByBias[bias].add(source);
        }
      });
      ...
      if (counts.LEFT + counts.CENTER + counts.RIGHT === 0 && group.evidence?.length) {
        group.evidence.forEach((item) => {
          if (item.bias === "LEFT" || item.bias === "CENTER" || item.bias === "RIGHT") {
            counts[item.bias] += 1;
            srcsByBias[item.bias].add(item.source ?? "Unknown");
          }
        });
      }
      ...
      if (!fallbackSeen[targetBias].has(claim)) {
        ...
        buckets[targetBias].push({ claim, sources: Array.from(srcsByBias[targetBias]), weight: counts.LEFT + counts.CENTER + counts.RIGHT });
      }
    });
    ...
    return data.claim_groups
      .map((group, index) => {
        ...
        return {
          ...
          evidenceCount,
          confidencePct,
          isStrong: group.verdict === "CONTRADICTED" || contradictionHints > 0,
        };
      })
  ...
  const contradictionFallbackCards = useMemo(() => {
    if (!data || topContradictions.length > 0) return [] as Array<{
      articleTitle: string;
    ...
    return data.articles.slice(0, 3).map((article) => {
      const contradiction = articleTopContradictions.get(getArticleKey(article));
      return {
        articleTitle: article.title ?? "Untitled article",
        articleSource: article.source ?? "Unknown source",
        claim: contradiction?.claim ?? "No strong contradiction detected for this source in the current evidence set.",
        score: contradiction?.score ?? 0,
      };
    });
  }, [articleTopContradictions, data, topContradictions.length]);
              ...
              <section className="space-y-8">
                ...
                <div className="rounded-2xl border border-parchment-300 bg-white/85 p-6 shadow-sm dark:border-[#2a313d] dark:bg-[#151922]/80">
                  ...
                  <p className="mb-5 text-xs text-ink-muted dark:text-[#8892a3]">
                    Points extracted from Left, Center, and Right leaning sources with source attribution.
                  </p>

                  <div className="grid gap-4 md:grid-cols-3">
                    {([
                      { key: "LEFT" as const, label: "Left", headingClass: "text-bias-left", dotClass: "bg-bias-left", borderClass: "border-bias-left", badgeClass: "bias-badge-left" },
                      { key: "CENTER" as const, label: "Center", headingClass: "text-ink-secondary dark:text-[#adb7c7]", dotClass: "bg-bias-center", borderClass: "border-parchment-300 dark:border-[#546076]", badgeClass: "bias-badge-center" },
                      { key: "RIGHT" as const, label: "Right", headingClass: "text-bias-right", dotClass: "bg-bias-right", borderClass: "border-bias-right", badgeClass: "bias-badge-right" },
                    ]).map((column) => {
                            ...
                            <ul className="space-y-3">
                              {points.slice(0, 6).map((point, index) => (
                                <li
                                  ...
                                  className="rounded-lg border border-parchment-200/60 bg-white/50 p-2.5 dark:border-[#202631]/60 dark:bg-[#111317]/40"
                                >
                                  <div className="flex gap-2 text-sm leading-relaxed text-ink-secondary dark:text-[#adb7c7]">
                                  ...
                                  {point.sources.length > 0 && (
                                    <div className="mt-1.5 flex flex-wrap gap-1 pl-3.5">
                                      {point.sources.slice(0, 3).map((src, si) => (
                                        <span
                                          key={`${column.key}-src-${index}-${si}`}
                                          className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[9px] font-semibold tracking-wide opacity-80 bias-badge ${column.badgeClass}`}
                                        >
                                      ...
                                      {point.sources.length > 3 && (
                                        <span className="text-[9px] text-ink-muted dark:text-[#8892a3]">+{point.sources.length - 3} more</span>
                                      )}
                                    </div>
                                  )}
                                </li>
                ...
                <div className="rounded-2xl border border-parchment-300 bg-white/85 p-6 shadow-sm dark:border-[#2a313d] dark:bg-[#151922]/80">
                  ...
                  {topContradictions.length === 0 ? (
                    contradictionFallbackCards.length === 0 ? (
                      <p className="text-sm text-ink-muted dark:text-[#8892a3]">No contradiction signals available for this query.</p>
                    ) : (
                      <div className="space-y-3">
                        {contradictionFallbackCards.map((item, index) => (
                          <div
                            key={`contradiction-fallback-${index}`}
                            className="rounded-xl border border-red-400/20 bg-red-500/5 p-3 dark:border-red-500/25 dark:bg-red-500/10"
                          >
                            <div className="mb-1 flex items-center gap-2">
                              ...
                              <span className="text-[10px] text-ink-muted dark:text-[#8892a3]">Score {item.score}</span>
                            </div>
                            <p className="text-[11px] text-ink-muted dark:text-[#8892a3]">
                              {item.articleSource} • {item.articleTitle}
                            </p>
                            <p className="mt-1 text-sm font-medium text-ink dark:text-[#eef1f8]">{item.claim}</p>
                          </div>
                        ))}
                      </div>
                    )
                      ...
                      {topContradictions.map((item, position) => (
                        ...
                        >
                          <div className="mb-1 flex flex-wrap items-center gap-2">
                            ...
                            {!item.isStrong && (
                              <span className="inline-flex rounded-full border border-amber-400/30 bg-amber-500/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-300">
                                weak signal
                              </span>
                            )}
                ...
                {/* Consensus Report */}
                {data.consensus_report && (
                  <ConsensusReportSection report={data.consensus_report} />
                )}
                      ...
                      {data.claim_groups.map((group, index) => (
                        <div key={`${group.representative_claim}-${index}`} id={`claim-group-${index}`} className="scroll-mt-24">
                          <ClaimGroup
                            ...
                            sourceBiases={group.source_biases}
                            biasBreakdown={group.bias_breakdown}
...
/** Structured consensus report section */
function ConsensusReportSection({ report }: { report: ConsensusReport }) {
  const reliabilityStyles: Record<string, string> = {
    ...
    UNCERTAIN: "border-slate-500/30 bg-slate-500/10 text-slate-700 dark:text-slate-300",
  };
  ...
  return (
    <div className="rounded-2xl border border-parchment-300 bg-white/85 p-6 shadow-sm dark:border-[#2a313d] dark:bg-[#151922]/80">
      <div className="mb-5 flex items-start justify-between gap-3">
        <div>
          <h3 className="font-serif text-xl text-ink dark:text-[#eef1f8]">
            Consensus Report
          </h3>
          <p className="mt-1 text-xs text-ink-muted dark:text-[#8892a3]">
            Structured analysis summary across all sources and perspectives.
          </p>
        </div>
        <span className={`shrink-0 rounded-full border px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest ${reliabilityStyles[report.reliability] ?? reliabilityStyles.UNCERTAIN}`}>
          {report.reliability} Reliability
        </span>
      </div>

      {/* Reliability description */}
      <div className="mb-5 rounded-xl border border-parchment-200 bg-parchment/40 p-4 dark:border-[#202631] dark:bg-[#111317]/40">
        <p className="text-sm leading-relaxed text-ink-secondary dark:text-[#adb7c7]">
          {report.reliability_description}
        </p>
        <div className="mt-3 flex flex-wrap items-center gap-4 text-xs text-ink-muted dark:text-[#8892a3]">
          <span>Score: <strong className="text-ink dark:text-[#eef1f8]">{report.consensus_score}%</strong></span>
          <span>Claims analyzed: <strong className="text-ink dark:text-[#eef1f8]">{report.total_claims_analyzed}</strong></span>
          <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${report.spectrum_complete ? "border-emerald-500/25 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "border-amber-500/25 bg-amber-500/10 text-amber-700 dark:text-amber-300"}`}>
            {report.spectrum_coverage}
          </span>
        </div>
      </div>

      {/* Verdict distribution mini-bars */}
      <div className="mb-5 grid grid-cols-4 gap-3">
        {([
          { key: "supported", label: "Supported", color: "bg-emerald-500", textColor: "text-emerald-700 dark:text-emerald-300" },
          { key: "contradicted", label: "Contradicted", color: "bg-red-500", textColor: "text-red-700 dark:text-red-300" },
          { key: "mixed", label: "Mixed", color: "bg-amber-500", textColor: "text-amber-700 dark:text-amber-300" },
          { key: "uncertain", label: "Uncertain", color: "bg-slate-400", textColor: "text-slate-600 dark:text-slate-300" },
        ] as const).map((item) => {
          const count = report.verdict_distribution[item.key];
          const total = Math.max(1, report.total_claims_analyzed);
          const pct = Math.round((count / total) * 100);
          return (
            <div key={item.key} className="text-center">
              <p className={`text-lg font-bold ${item.textColor}`}>{count}</p>
              <div className="mx-auto my-1 h-1.5 w-full overflow-hidden rounded-full bg-parchment-200 dark:bg-[#202631]">
                <div className={`h-full rounded-full transition-all duration-700 ${item.color}`} style={{ width: `${Math.max(pct > 0 ? 8 : 0, pct)}%` }} />
              </div>
              <p className="text-[10px] uppercase tracking-wide text-ink-muted dark:text-[#8892a3]">{item.label}</p>
            </div>
          );
        })}
      </div>
      ...
      {report.key_findings.length > 0 && (
        <div className="mb-5">
          <h4 className="mb-2.5 text-sm font-semibold text-ink dark:text-[#eef1f8]">Key Findings</h4>
          <ul className="space-y-2">
            {report.key_findings.map((finding, i) => (
              <li key={`finding-${i}`} className="flex gap-2.5 text-sm leading-relaxed text-ink-secondary dark:text-[#adb7c7]">
      ...
      {/* Cross-spectrum claims */}
      {report.cross_spectrum_claims.length > 0 && (
        <div className="mb-5">
          <h4 className="mb-2.5 text-sm font-semibold text-ink dark:text-[#eef1f8]">
            Cross-Spectrum Claims
            <span className="ml-2 text-[10px] font-normal text-ink-muted dark:text-[#8892a3]">
              Reported across {report.cross_spectrum_claims.length > 1 ? "multiple" : "different"} bias perspectives
            </span>
          </h4>
          <div className="space-y-2">
            {report.cross_spectrum_claims.map((item, i) => (
              <div key={`cross-${i}`} className="rounded-xl border border-parchment-200 bg-parchment/40 p-3 dark:border-[#202631] dark:bg-[#111317]/40">
                <p className="text-sm font-medium text-ink dark:text-[#eef1f8]">&ldquo;{item.claim}&rdquo;</p>
                <div className="mt-2 flex items-center gap-3">
                  <div className="flex h-1.5 flex-1 overflow-hidden rounded-full">
                    {(["LEFT", "CENTER", "RIGHT"] as const).map((bias) => {
                      const total = item.bias_breakdown.LEFT + item.bias_breakdown.CENTER + item.bias_breakdown.RIGHT;
                      const pct = total > 0 ? (item.bias_breakdown[bias] / total) * 100 : 0;
                      return pct > 0 ? (
                        <div key={bias} className={`${biasColors[bias]} transition-all duration-500`} style={{ width: `${pct}%` }} />
                      ) : null;
                    })}
                  </div>
                  <span className="text-[10px] text-ink-muted dark:text-[#8892a3]">
      ...
      {/* Top agreed claims */}
      {report.top_agreed_claims.length > 0 && (
        <div className="mb-5">
          <h4 className="mb-2.5 text-sm font-semibold text-ink dark:text-[#eef1f8]">
            Top Agreed Claims
            <span className="ml-2 text-[10px] font-normal text-ink-muted dark:text-[#8892a3]">
          ...
          <div className="space-y-2">
            {report.top_agreed_claims.map((item, i) => (
              <div key={`agreed-${i}`} className="flex items-start gap-3 rounded-xl border border-emerald-200/40 bg-emerald-500/5 p-3 dark:border-emerald-500/20 dark:bg-emerald-500/5">
                <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-500/15 text-[10px] font-bold text-emerald-700 dark:text-emerald-300">
                  {item.source_count}
                </span>
                <div className="flex-1">
                  <p className="text-sm font-medium text-ink dark:text-[#eef1f8]">{item.claim}</p>
                  <div className="mt-1 flex flex-wrap gap-1">
                    {item.sources.slice(0, 4).map((src, si) => (
                      <span key={`agreed-src-${i}-${si}`} className="text-[10px] text-ink-muted dark:text-[#8892a3]">
                        {src}{si < Math.min(3, item.sources.length - 1) ? " •" : ""}
                      </span>
                    ))}
                    {item.sources.length > 4 && (
                      <span className="text-[10px] text-ink-muted dark:text-[#8892a3]">+{item.sources.length - 4} more</span>
                    )}
                  </div>
                </div>
              </div>
      ...
      {report.high_confidence_claims.length > 0 && (
        <div>
          <h4 className="mb-2.5 text-sm font-semibold text-ink dark:text-[#eef1f8]">
            High Confidence Claims
            <span className="ml-2 text-[10px] font-normal text-ink-muted dark:text-[#8892a3]">
              70%+ verification confidence
            </span>
          </h4>
          <div className="space-y-2">
            {report.high_confidence_claims.map((item, i) => {
              const verdictBadge =
                item.verdict === "SUPPORTED" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"
                : item.verdict === "CONTRADICTED" ? "border-red-500/30 bg-red-500/10 text-red-700 dark:text-red-300"
                : item.verdict === "MIXED" ? "border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-300"
                : "border-slate-500/30 bg-slate-500/10 text-slate-700 dark:text-slate-300";
              return (
                <div key={`hc-${i}`} className="rounded-xl border border-parchment-200 bg-parchment/40 p-3 dark:border-[#202631] dark:bg-[#111317]/40">
                  <div className="mb-1.5 flex flex-wrap items-center gap-2">
                    <span className={`inline-flex rounded-full border px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide ${verdictBadge}`}>
                      {item.verdict}
                    </span>
                    <span className="text-[10px] text-ink-muted dark:text-[#8892a3]">
                      Confidence {Math.round(item.confidence)}% • {item.source_count} source{item.source_count !== 1 ? "s" : ""}
                    </span>
                  </div>
                  <p className="text-sm font-medium text-ink dark:text-[#eef1f8]">{item.claim}</p>
                </div>
