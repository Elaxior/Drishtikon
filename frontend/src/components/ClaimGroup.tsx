...
type ClaimGroupProps = {
  ...
  sourceBiases?: Array<{ source: string; bias: BiasType }>;
  biasBreakdown?: { LEFT: number; CENTER: number; RIGHT: number; UNKNOWN: number };
  count: number;
  verdict?: VerdictType;
  confidence?: number;
  reason?: string;
...
function BiasBreakdownBar({ breakdown }: { breakdown: { LEFT: number; CENTER: number; RIGHT: number; UNKNOWN: number } }) {
  const total = breakdown.LEFT + breakdown.CENTER + breakdown.RIGHT;
  if (total === 0) return null;

  const leftPct = Math.round((breakdown.LEFT / total) * 100);
  const centerPct = Math.round((breakdown.CENTER / total) * 100);
  const rightPct = 100 - leftPct - centerPct;

  return (
    <div className="mt-2">
      <div className="mb-1 flex items-center gap-3 text-[10px] text-ink-muted dark:text-[#8892a3]">
        {breakdown.LEFT > 0 && (
          <span className="flex items-center gap-1">
            <span className="h-2 w-2 rounded-full bg-bias-left" />
            Left ({breakdown.LEFT})
          </span>
        )}
        {breakdown.CENTER > 0 && (
          <span className="flex items-center gap-1">
            <span className="h-2 w-2 rounded-full bg-bias-center" />
            Center ({breakdown.CENTER})
          </span>
        )}
        {breakdown.RIGHT > 0 && (
          <span className="flex items-center gap-1">
            <span className="h-2 w-2 rounded-full bg-bias-right" />
      ...
      <div className="flex h-1.5 overflow-hidden rounded-full">
        {leftPct > 0 && (
          <div className="bg-bias-left transition-all duration-500" style={{ width: `${leftPct}%` }} />
        )}
        {centerPct > 0 && (
          <div className="bg-bias-center transition-all duration-500" style={{ width: `${centerPct}%` }} />
        )}
        {rightPct > 0 && (
          <div className="bg-bias-right transition-all duration-500" style={{ width: `${rightPct}%` }} />
        )}
      </div>
    </div>
  );
    ...
    <div className="rounded-xl border border-parchment-200 bg-parchment/70 p-4 transition-all duration-200 hover:border-parchment-300 dark:border-[#202631] dark:bg-[#151922]/60 dark:hover:border-[#2a313d]">
      <div className="mb-3 flex items-start gap-3">
        ...
        <p className="flex-1 text-sm font-medium leading-snug text-ink dark:text-[#eef1f8]">
          &ldquo;{representativeClaim}&rdquo;
        </p>
      </div>
      ...
      {/* Source badges with bias */}
      <div className="flex flex-wrap items-center gap-2">
        {(sourceBiases ?? sources.map(s => ({ source: s.name, bias: s.bias }))).map((sb, index) => (
          <span
            key={`${sb.source}-${sb.bias}-${index}`}
            className={getBiasBadgeClass(sb.bias)}
          >
            {sb.source}
          </span>
        ))}
      ...
      {/* Bias breakdown mini-bar */}
      {biasBreakdown && <BiasBreakdownBar breakdown={biasBreakdown} />}
