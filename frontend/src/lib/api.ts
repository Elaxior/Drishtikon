...
export type ConsensusReport = {
  reliability: "HIGH" | "MODERATE" | "LOW" | "UNCERTAIN";
  ...
  key_findings: string[];
  top_agreed_claims: Array<{
    claim: string;
    source_count: number;
    sources: string[];
  }>;
  cross_spectrum_claims: Array<{
    claim: string;
    bias_breakdown: { LEFT: number; CENTER: number; RIGHT: number; UNKNOWN: number };
    source_count: number;
    spectrum_breadth: number;
  }>;
  high_confidence_claims: Array<{
    claim: string;
    confidence: number;
    verdict: string;
    source_count: number;
  }>;
  spectrum_coverage: string;
  spectrum_complete: boolean;
};
